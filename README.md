...
Isso cria uma rede isolada permitindo que todas as aplicações conversem com o banco na porta interna `5432` (exposta na porta `55432` para máquina local).
...
Localize a DAG `analytics_pipeline` e clique no botão de **"Trigger DAG"** (ícone de play) para iniciar a execução.
...
```bash
# Criar Views (Silver) e Tabelas (Gold)
docker compose exec -T airflow bash -lc "cd /opt/project/dbt && dbt run --profiles-dir /opt/project/dbt"

# Rodar asserções de qualidade de dados
docker compose exec -T airflow bash -lc "cd /opt/project/dbt && dbt test --profiles-dir /opt/project/dbt"
```
